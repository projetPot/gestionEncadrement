<?php/* c'est la page session qui sera inclue dans toutes les pages du site si une session est ouverte. Elle 
contient toutes les information necessaires pour faire comprendre à un individu qu'il est deja connecté:
telles que son pseudo, son avatar(photo) et un bouton deconnection.*/?>
	<div style="position: fixed; top: 10px; left: 10px; width: 170px; height: 150px;" id="ident">
			 <?php if(isset ($_SESSION['name']) &&  $_SESSION['photo']!=""){ echo ' <img title='.$_SESSION['name'].' width=\'120\' src=\'../avatar/'.$_SESSION['photo'].'\' />  <br /> <font color=\'yellow\'> <strong> Bienvenu <u> '.$_SESSION['name'].'</u> </strong> </font>';} 
										 ...
										 <br /> <div id = "setting" style=" width: 120px; text-align: center; cursor: pointer;" onclick="affi('parametres','setting');" > 
												...
												<div id="parametres" style=" width: 120px; border: blue solid 1px; background-color: #EEECEC; display: none; "> 
													<div class="setting" id = "setting" style=" text-align: center;" >  
														<img  src="../images/setting.png" style="width: 20px; height: 20px; float: left; margin-top: 1px; margin-left: 2px;" />  settings 
													</div> 
	...
	<input type="hidden" value="valeur();" id="hidde" />
	<br /> <br />
	
<script>
flag = document.getElementById('hidde');
function hidshow(){
alert(flag.value);
if(flag.value == 0){
location = 'hidden.php';
